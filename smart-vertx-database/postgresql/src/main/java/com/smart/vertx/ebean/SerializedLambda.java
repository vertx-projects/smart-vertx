...
import com.smart.vertx.ebean.util.ClassUtils;
import com.smart.vertx.ebean.util.SerializationUtils;
...
public class SerializedLambda implements Serializable {
    ...
    public static SerializedLambda resolve(SFunction<?, ?> lambda) {
        if (!lambda.getClass().isSynthetic()) {
            throw new VertxResCriteriaException("该方法仅能传入 lambda 表达式产生的合成类");
        }
        try (ObjectInputStream objIn = new ObjectInputStream(new ByteArrayInputStream(SerializationUtils.serialize(lambda))) {
            ...
            throw new VertxResCriteriaException("This is impossible to happen", e);
        }
    }
